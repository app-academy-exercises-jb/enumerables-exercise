...
def substrings(string)
  substrings = []
  string.each_char.with_index { |c, i| string[i..-1].each_char.with_index { |c2, i2| 
    substrings << string[i..i2+i]
  } }
  substrings
end
...
def doubler(array)
  raise TypeError.new("only works on numeric arrays") unless array.all? { |ele| ele.kind_of?(Numeric) }
  array.map { |i| i * 2 }
end
...
class Array
  def my_map(&prc)
    self.my_inject([]) { |acc, ele| acc << prc.call(ele); acc }
  end

  def my_select(&prc)
    self.my_reduce([]) { |acc, ele| acc << ele if prc.call(ele); acc }
  end

  def my_reduce(*args, &blck) #yeah, we went for the fancy version :) my_reduce should has all of the native functionality of reduce
    arr = self.to_a
    case args.length
    when 0
...
def concatenate(strings)
  strings.my_inject("") { |acc, str| acc += str; acc }
end